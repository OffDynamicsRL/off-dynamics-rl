...
def call_mujoco_env(env_config: Dict) -> gym.Env:
    ...
    if 'friction' in env_name or 'gravity' in env_name:
        assert float(shift_level) in [0.1, 0.5, 2.0, 5.0], 'The required shift is not available yet, please consider modify the xml file on your own or use the shift scale among 0.1, 0.5, 2.0, 5.0'
