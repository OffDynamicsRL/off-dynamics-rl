...
class H2O(object):
    ...
    def update_q_functions(self, state_batch, action_batch, reward_batch, nextstate_batch, not_done_batch, writer=None):
        with torch.no_grad():
            ...
            if self.config['backup_entropy']:
                ...
                value_target = reward_batch + not_done_batch * self.discount * q_target.unsqueeze(dim=1)
    ...
    def update_cql_q_functions(self, state_batch, action_batch, reward_batch, nextstate_batch, not_done_batch, writer=None):
        ...
        with torch.no_grad():
            ...
            importance_weighting  = torch.clip(importance_weighting, min=1e-4, max=10.)
        ...
        cql_qf1_diff = torch.clamp(cql_qf1_ood - q_1, self.config['cql_clip_diff_min'], self.config['cql_clip_diff_max'])
        cql_qf2_diff = torch.clamp(cql_qf2_ood - q_2, self.config['cql_clip_diff_min'], self.config['cql_clip_diff_max'])

        if self.config['cql_lagrange']:
            alpha_prime = torch.clamp(self.alpha_prime, min=0.0, max=1000000.0)
            cql_min_qf1_loss = (alpha_prime * importance_weighting * self.config['cql_alpha'] * (cql_qf1_diff - self.config['cql_target_action_gap'])).mean()
            cql_min_qf2_loss = (alpha_prime * importance_weighting * self.config['cql_alpha'] * (cql_qf2_diff - self.config['cql_target_action_gap'])).mean()
            self.temp_prime_optimizer.zero_grad()
            alpha_prime_loss = (-cql_min_qf1_loss - cql_min_qf2_loss) * 0.5
            alpha_prime_loss.backward(retain_graph=True)
            self.temp_prime_optimizer.step()
        else:
            cql_min_qf1_loss = (cql_qf1_diff * self.config['cql_alpha'] * importance_weighting).mean()
            cql_min_qf2_loss = (cql_qf2_diff * self.config['cql_alpha'] * importance_weighting).mean()
