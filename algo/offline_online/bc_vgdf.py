...
class BCVGDF:
    ...
    def train(self, src_replay_buffer, tar_replay_buffer, batch_size=128, writer=None) -> None:
        ...
        if self.training_count % self.training_delay == 0:
            # train policy
            loss_policy, new_a_log_prob = self.update_policy(
                torch.cat([src_s, tar_s], dim=0),
                src_s = src_s,
                src_a = src_a,
            )
    ...
    def update_policy(self, s: torch.tensor, src_s: torch.tensor, src_a: torch.tensor,) -> torch.tensor:
