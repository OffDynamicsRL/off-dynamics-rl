...
class SACBC(object):
    ...
    def update_policy_and_temp(self, state_batch, tar_state_batch, tar_action_batch):
        ...
        pred_tar_act, _, _ = self.policy(tar_state_batch, get_logprob=True)

        policy_loss = p_w * (self.alpha * logprobs_batch - qval_batch).mean() + F.mse_loss(pred_tar_act, tar_action_batch)
